...
class AudioResNet(object):
    ...
    def build_network(self): #定义神经网络
        ...
        def aconv1d_layer(input_tensor, size, rate, activation, scale, bias):
            with tf.variable_scope('aconv1d_' + str(self.aconv1d_index)):
                ...
                out = tf.nn.atrous_conv2d(tf.expand_dims(input_tensor, axis=1), W, rate=rate, padding='SAME')
    ...
    def train(self, folder, text, batch_size, saver_folder = './model/speech'):
        print "开始训练:", time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
        audio_batch = AudioBatch(folder, text, self.input_seqs, self.output_size, batch_size)
    ...
    def load(self, saver_folder = './model'):
        self.session = tf.Session()
        self.saver.restore(self.session, tf.train.latest_checkpoint(saver_folder))
    ...
    def sample(self, mfcc):
        ...
        output = self.session.run(decoded, feed_dict={self.inputs: mfcc})
        #print(output)
